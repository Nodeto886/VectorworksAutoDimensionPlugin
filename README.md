...
- 工具模式栏提供 `Object dimensions` 与 `Fixture spacing` 两种模式：前者独立标注每个所选对象，后者为相邻所选对象生成中心到中心尺寸，至少需要两个可测对象。
...
- 立面视图下 `Fixture spacing` 用投影后的中心点计算最近邻，符号和参数对象仍以插入点为中心。
...
5. 在模式栏选择 `Object dimensions`，确认每个所选对象被独立标注，源对象没有被复制。
6. 选择至少两个对象并切换到 `Fixture spacing`，确认只生成连接相邻对象的中心到中心尺寸，不生成所有对象两两组合。
